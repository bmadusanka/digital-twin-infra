...
variable "target_bucket_id" {
  description = "Target bucket to push logs"
  type        = string
}
...
variable "target_object_key_format" {
  description = "Target object keys format."
  default = {
    enabled = false
  }
  type = object({
...
#### Transition lifecycle rule for current versions ####
...
#### Expiration lifecycle rule for current versions ####
